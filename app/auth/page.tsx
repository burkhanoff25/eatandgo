...
import { ArrowLeft, Phone, Lock, User, Calendar, ShieldCheck } from 'lucide-react';
import toast from 'react-hot-toast';
import { useBonus } from '../../hooks/useBonus';

export default function AuthPage() {
  const router = useRouter();
  const { user, sendOTP, login, signInWithGoogle, loading: authLoading } = useBonus();
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await sendOTP(phone, isRegistering ? name : undefined, isRegistering ? birthday : undefined);
      toast.success('Код отправлен (используйте 1234 для проверки)');
      setStep('otp');
    } catch (err: any) {
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-[#121212] flex items-center justify-center">
        <div className="w-10 h-10 border-4 border-primary-red border-t-transparent rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#121212] flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="max-w-md w-full mx-auto px-4">
        {/* Back Link */}
        <Link href="/" className="inline-flex items-center space-x-2 text-white/60 hover:text-white mb-8 text-sm transition-colors">
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-2xl border border-gray-100">
          {/* Header */}
          <div className="bg-brand-dark p-8 text-center text-white border-b border-white/5">
          ...
          {step === 'phone' ? (
            <form onSubmit={handleSendOTP} className="p-6 sm:p-8 space-y-6">
              <div className="text-center space-y-2">
                ...
                <p className="font-body text-xs text-gray-400 font-semibold leading-relaxed">
                  Введите номер телефона для входа или регистрации в клубе.
                </p>
              </div>
            ...
            <form onSubmit={handleVerifyOTP} className="p-6 sm:p-8 space-y-6">
              <div className="text-center space-y-2">
                ...
                <p className="font-body text-xs text-gray-400 font-semibold leading-relaxed">
                  Введите 4-значный код, отправленный на <strong className="text-brand-dark">{phone}</strong>.
                </p>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider block text-center">Код из СМС</label>
